...
<div class="leaf">
  ...
  nextButton.addEventListener("click", () => {
    isRandomMode && scrollToText("Яна"); // понимаемся всегда сначала наверх // TODO заменить на 1й lane в Кайтене

    // через 0.7 секунды идем вниз к доске выступающего
    setTimeout(() => {
      scrollToText(randomList[0]);

      // костыль для корректного скролла наверх к Яне
      if (randomList[0] === "Яна") {
        // временная функция для поднятия к Яне в начало
        function yanaUp() {
      ...
      if (randomList.length > 0) {
        if (randomList.length === 1) {
          randomList[0] = `${randomList[0]} (это последний спикер)`; // подсветить последнего
          nextButton.disabled = true;
        }

        nextSpeakerField.textContent = randomList[0]; // показываем первый элемент
